...
func TestDriver_ValidatePluginCapability(t *testing.T) {
	tests := []struct {
		name    string
		driver  DiskDriver
	...
	}{
		{
			...
			cap:     csi.PluginCapability_Service_VOLUME_ACCESSIBILITY_CONSTRAINTS, //support topology
			isValid: true,
		},
		{
	...
	for _, test := range tests {
		...
		if test.isValid != isValid {
			t.Errorf("name %s: expect %t but actually %t", test.name, test.isValid, isValid)
		}
	}
}

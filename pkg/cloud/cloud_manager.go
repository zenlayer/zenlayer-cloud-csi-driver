...
// nilClientMsg is reported when a cloud API is invoked on a manager that has no
// zec client. NewZecCloudManager leaves zecClient nil for the node driver on
// purpose (a node holds no cloud credentials), so every entry point that talks to
// the cloud checks it first and fails with this message instead of panicking on a
// nil pointer dereference.
const nilClientMsg = "zec cloud client is not initialized, node driver cannot call cloud api"
...
func (cm *zecCloudManager) Probe() error {
	...
	if cm.zecClient == nil {
		return errors.New(ERRORLOG + nilClientMsg)
	}
...
func (cm *zecCloudManager) GetZoneList() ([]string, error) {
	...
	if cm.zecClient == nil {
		return nil, errors.New(ERRORLOG + nilClientMsg)
	}
...
Action: delete cloud disk.
	...
	第一次ReleaseDisk只是把云盘放入回收站(RECYCLED),需要对处于RECYCLED状态的云盘
	再次ReleaseDisk才会真正销毁,所以最多需要释放maxReleaseDiskTimes次。
	每次释放前先查询云盘当前状态:
	  - 云盘已不存在: 说明已经彻底销毁,幂等返回成功。
	  - 云盘已处于RECYCLED: 本次释放就是最后一次,直接等待云盘消失。
	  - 其它状态: 本次释放后等待云盘进入回收站(或直接消失),再进入下一轮。
	这样即使上一次DeleteVolume在两次释放之间失败退出,重试时也能从回收站状态接着删完。
...
func (cm *zecCloudManager) DeleteVolume(volId string) (err error) {
	...
	for times := 1; times <= maxReleaseDiskTimes; times++ {
		volInfo, err := cm.FindVolume(volId)
		if err != nil {
			klog.Errorf("%s find volume error[%v], volId[%s], release times[%d]", ERRORLOG, err.Error(), volId, times)
			return err
		}
		//云盘已经彻底销毁
		if volInfo == nil {
			klog.Infof("%s volId[%s] not exist, release done. release times[%d]", INFOLOG, volId, times-1)
			return nil
		}
		//本次释放前云盘就已经在回收站里, 说明这次释放就是最后一次
		lastRelease := volInfo.ZecVolume_Status == DiskStatusRecycled

		request := zec.NewReleaseDiskRequest()
		request.DiskId = &volId

		klog.Infof("%s Will release volId[%s], current status[%s], release times[%d]", INFOLOG, volId, volInfo.ZecVolume_Status, times)
		_, err = cm.zecClient.ReleaseDisk(request)
		if err != nil {
			//SDK 只在真正拿到云端应答(或解析应答失败)时才把错误包成 *ZenlayerCloudSdkError;
			//调用方 context 被取消、构造 http.Request 失败等情况会把底层 error 原样返回。
			//所以这里必须用带 ok 的断言, 断言失败时按普通错误处理, 不能直接取 .Code。
			if sdkErr, ok := err.(*common.ZenlayerCloudSdkError); ok && sdkErr.Code == OPERATION_FAILED_RESOURCE_NOT_FOUND {
				klog.Infof("%s API INFO, volId[%s] Not Found", INFOLOG, volId)
				return nil
			}
			klog.Errorf("%s API ERROR, error[%v], volId[%s], release times[%d]", ERRORLOG, err.Error(), volId, times)
			return err
		}

		if lastRelease {
			//回收站中的云盘再次释放后会被真正销毁, 等待其从控制台消失
			if err = cm.waitDiskStatus(DiskStatusDeleted, volId); err != nil {
				return err
			}
			klog.Infof("%s Succeed delete volId[%s], release times[%d]", INFOLOG, volId, times)
			return nil
		}

		//等待本次释放生效: 云盘要么直接消失, 要么进入回收站等待下一次释放
		gone, err := cm.waitDiskReleased(volId)
		if err != nil {
			return err
		}
		if gone {
		...
		klog.Infof("%s volId[%s] is in recycle bin after release times[%d], will release again", INFOLOG, volId, times)
	}

	klog.Errorf("%s volId[%s] still exist after release %d times", ERRORLOG, volId, maxReleaseDiskTimes)
	return fmt.Errorf("%s volId[%s] still exist after release %d times", ERRORLOG, volId, maxReleaseDiskTimes)
}
...
func (cm *zecCloudManager) DetachVolume(volId string) (err error) {
	...
	//DetachDisks可能返回成功但把云盘放进failedDiskIds表示本次解绑没生效,
	//此时err为nil, 不检查就会白等waitDiskStatus一整个超时且丢掉失败原因。
	//response为nil时不当作失败: 下面的waitDiskStatus才是解绑是否生效的最终判据。
	if response.Response != nil && len(response.Response.FailedDiskIds) != 0 {
		klog.Errorf("%s API ERROR, detach disk fail, volId[%s], failedDiskIds[%v]", ERRORLOG, volId, response.Response.FailedDiskIds)
		return fmt.Errorf("%s API ERROR, DetachDisks response failedDiskIds is not nil, req volId[%s], response failedDiskIds[%v]", ERRORLOG, volId, response.Response.FailedDiskIds)
	}
...
zenlayer sdk describe(v0.2.49):
	...
	type DescribeDisksRequest struct {
	    *common.BaseRequest
	    ...
	    PageSize *int `json:"pageSize,omitempty"`// PageSize 返回的分页大小。// 默认为20，最大为1000。
...
func (cm *zecCloudManager) FindVolumeByName(volName string, zoneID string, sizeGB int, diskType string, ResourceGroupId string) (*ZecVolume, error) {
	...
	for c := 0; c < len(DiskListAll); c++ {
		...
		//DescribeDisks会连回收站里的云盘一起返回(DeleteVolume的分阶段释放正依赖这一点),
		//但回收站里的云盘不能再被挂载使用。若把它当成"同名卷已存在"回给CO, PV会建出来
		//而后续attach必然失败, 所以按名字查找可用卷时要跳过它。
		if zecVolInfos[c].ZecVolume_Status == DiskStatusRecycled {
			klog.Infof("%s skip recycled volume, volumeid[%s], volumename[%s], status[%s]", INFOLOG, zecVolInfos[c].ZecVolume_Id, zecVolInfos[c].ZecVolume_Name, zecVolInfos[c].ZecVolume_Status)
			continue
		}
...
// DescribeInstancesRequest
		...
		type DescribeInstancesRequest struct {
		    *common.BaseRequest
		    ...
		    Ipv4Address *string `json:"ipv4Address,omitempty"`// Ipv4Address 根据实例关联的IPv4过滤。// 若指定为网段(CIDR)形式，掩码长度必须在20–31之间。
...
// InstanceInfo 描述虚拟机实例的信息。包括规格，状态，网卡等。
	...
	type InstanceInfo struct {
		...
		ZoneId *string `json:"zoneId,omitempty"`// ZoneId 实例所属的可用区ID。
		InstanceType *string `json:"instanceType,omitempty"` // InstanceType 规格唯一ID。
		...
		PlacementGroupId *string `json:"placementGroupId,omitempty"`// PlacementGroupId 实例所属的置放组ID。
...
// ZoneInfo 可用区的基本信息。
	...
	type ZoneInfo struct {
	    ...
	    TimeZone *string `json:"timeZone,omitempty"`// TimeZone 可用区所在的时区。
	    CityName *string `json:"cityName,omitempty"` // CityName 可用区所在的城市名称。
	    CityCode *string `json:"cityCode,omitempty"`// CityCode 城市三字码。
	    CountryCode *string `json:"countryCode,omitempty"`// CountryCode 可用区所在的国家，ISO 3166-1 alpha-2 两字母代码（如 US、JP）。
	    CountryName *string `json:"countryName,omitempty"` // CountryName 可用区所在的国家名称。
	    SupportHaVip *bool `json:"supportHaVip,omitempty"`// SupportHaVip 可用区是否支持高可用虚拟IP（HaVip）。
	}
*/
...
func (cm *zecCloudManager) GetVmStatus(vmId string) (exist bool, status string, err error) {
	...
	if cm.zecClient == nil {
		return false, "", errors.New(ERRORLOG + nilClientMsg)
	}
...
func (cm *zecCloudManager) DeleteSnapshot(snapshotId string) (err error) {
	...
	if err != nil {
		//同 DeleteVolume: SDK 不保证所有错误都是 *ZenlayerCloudSdkError(context 取消、
		//构造 http.Request 失败会原样返回底层 error), 必须用带 ok 的断言。
		if sdkErr, ok := err.(*common.ZenlayerCloudSdkError); ok && sdkErr.Code == INVALID_DISK_SNAPSHOT_NOT_FOUND {
...
/*
Action: wait one ReleaseDisk Asynchronous operation done.

	ReleaseDisk之后云盘有两种终态: 直接从控制台消失(彻底销毁), 或者进入回收站(RECYCLED)
	等待再次释放。本函数等待其中任意一种终态出现, 由返回值gone区分, 供DeleteVolume
	判断是否还需要再释放一次。

Args: volId(string) //cloud disk ID

Return: gone(bool) //true=云盘已彻底销毁, false=云盘已进入回收站需要再次释放

	err error
*/
func (cm *zecCloudManager) waitDiskReleased(volId string) (gone bool, err error) {

	funcName := "zecCloudManager:waitDiskReleased:"
	funcInfo, hash := csicommon.EntryFunction(funcName)
	klog.Info(funcInfo)
	defer klog.Info(csicommon.ExitFunction(funcName, hash))
	ERRORLOG := "ERROR:" + funcName + hash + " "
	...
	if volId == "" {
		return false, errors.New(ERRORLOG + "error args" + ".Volid=" + volId)
	}
	...
	job := func() (stop bool, err error) {
		...
		if *response.Response.TotalCount != 1 {
			return true, errors.New(ERRORLOG + "VolId Repeat, volid=" + volId)
		}
		if len(response.Response.DataSet) == 0 || response.Response.DataSet[0] == nil || response.Response.DataSet[0].DiskStatus == nil {
			return true, errors.New(ERRORLOG + " nil response.volid=" + volId)
		}

		//云盘进入回收站, 需要再次ReleaseDisk才会真正销毁
		if *response.Response.DataSet[0].DiskStatus == DiskStatusRecycled {
			gone = false
			return true, nil
		}

		klog.Infof("%s Wait Disk Released 3 sec again.VolId[%s], status[%s]", INFOLOG, volId, *response.Response.DataSet[0].DiskStatus)
		return false, nil
	}

	if err = WaitFor(job); err != nil {
		return false, err
	}
	return gone, nil
}
